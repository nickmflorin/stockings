...
export const WrappedElement = (element: cheerio.Element | IWrappedElement): IWrappedElement =>
  typeof (element as IWrappedElement).find === "function"
    ? (element as IWrappedElement)
    : {
        ...(element as {
          [key in keyof cheerio.Element]: cheerio.Element[key];
        }),
        ...
        findText<O extends FindOptions>(this: IWrappedElement, options?: O) {
          ...
          if (elements.length > 1) {
            const err = new NonUniqueTextError({ parent: this });
            if (strictUniqueness) {
              throw err;
            }
            logger.warn(err.message, { parent: this.toString() });
        ...
        find<O extends FindOptions>(
          ...
          const elements = this.children.filter(
            ...
            (c): c is cheerio.Element =>
              isElement(c) && c.type === ElementType.Tag && sel.evaluate(c),
          );
          if (elements.length > 1) {
            const err = new NonUniqueElementError(sel, { parent: this });
            if (strictUniqueness) {
              throw err;
            }
