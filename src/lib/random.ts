...
type MultipleFrequencyDatumWithId<T> = {
  id: string;
  value: T;
  frequency: number;
  maxCount?: number;
 ...
 *   If multiple selections are desired, the options that include the 'length' of the resulting
 *   array.
 *
 * @returns {T | T[]}
 *
...
export function selectAtRandomFrequency<T>(
  ...
  const _selectAtRandomFrequency = <F extends FrequencyDatum<T>>(frequencies: F[]) => {
    if (frequencies.length === 0) {
      ...
      throw new Error(
        `Detected negative numbers in frequency data: ${JSON.stringify(frequencies)}. ` +
    ...
    const cumulative = normalized.map((d, i) => ({
      ...d,
    ...
    for (let i = 0; i < cumulative.length; i++) {
      const current = ensureItIsDefined(cumulative[i]);
  ...
  if (options) {
    /* In the case that multiple selections are being made, we need to be able to differentiate a
       given datum from another one, which allows us to determine if the maximum selection count for
       a given datum has been reached.  We cannot identify a datum based on the value, because the
       value may not be comparable with a shallow equality and it is possible that multiple datums
       exist with the same value.  So, each datum needs to be assigned a unique ID at the start -
       which will allow us to remove datums from the frequency population in the case that its max
       count was reached. */
    let runningFrequencies: MultipleFrequencyDatumWithId<T>[] = [...data].map(d => ({
      ...d,
      id: uuid(),
    }));
    let result: T[] = [];
    ...
    if (result.length < desiredLength) {
      logger.warn(
        `Could not select the desired number of elements, '${desiredLength}', from the provided ` +
          "data.  The maximum counts of each value in the dataset were reached before the " +
          "desired length was met.",
      );
    }
    return result;
  }
...
const isDuplicated = <T, V extends string | number>(
  prev: T[],
  value: T,
  duplicationKey: (v: T) => V,
): boolean => prev.some(p => duplicationKey(value) === duplicationKey(p));
...
export const selectAtRandomWithoutDuplication = <T, V extends string | number>(
  ...
  while (population.length > 0) {
    ...
    if (!datum) {
      ...
      /* If the selected value is a duplicate, simply remove that value from the selection
...
type RandomSelectionArrayOpts<T, V extends string | number> = {
  /**
   * Either the length that the randomly selected array should be or a minimum, maximum length
   * range that the resulting array should lie in.  The returned array will meet this length
   * criteria as long as there are sufficient elements in the array to select from.
   */
  length: RandomLength;
  /**
   * If provided, elements will be randomly selected and added to the array as long as they do not
   * represent duplicates of elements already in the array, where a duplicate is determined based
   * on an equality check of this function's return value.
   *
   * Elements will continue to be selected until the array reaches the desired length or until
   * there are no more elements to select that ensure uniqueness in the array.
   */
  duplicationKey?: (v: T) => V;
  /**
   * Whether or not an error should be thrown when there are no more unique elements to select
   * from and making a selection from the array would require introducing a duplicate value.  If
   * not provided, or provided as 'false', the array will be prematurely returned with the elements
   * that were selected up to that point, ensuring that there are no duplicates but also not
   * generating an array that meets the length requirement.
   *
...
/**
 * Creates an array of randomly selected elements, {@link T[]}, by making random selections from
 * the provided array, {@link T[]}, either until the maximum length is reached or until there are
 * no more unique elements to select from (in the case that the `duplicationKey` is provided).
 *
...
export const selectArrayAtRandom = <T, V extends string | number>(
  ...
  for (let i = 0; i < getLength(options.length); i++) {
    if (options.duplicationKey) {
      ...
      if (!selection) {
        /* Since the length of the array may have been randomly generated, we only want to throw
           an error in the case that no more unique elements can be constructed AND the number of
           already selected unique elements does not meet the minimum threshold.  If the length is
           provided as just a single number (i.e. no min/max or range) the required length will be
...
export const selectSequentially = <T>(data: T[], count = 0): T => {
  ...
  if (datum === undefined) {
    throw new Error(
      `Data unexpectedly does not have value at index '${count}', data length was '${data.length}'.`,
    );
  }
  return datum;
};
...
export function* infiniteLoopGenerator<T, V extends string | number>(
  ...
  for (let i = 0; i < getLength(options.length); i++) {
    const value = getter();
    /* If there is a duplication key, keep iterating until a duplicate is found - at that point in
       time, we will have gone through the entire iterable (assuming there are no duplicates in the
       iterable). */
